...
ERGONEKTIM is an auditable product that converts causal electrical observations into explicit, distributive, and scientifically traceable viability diagnostics.
...
- a canonical diagnostic-signal contract;
- a [universal closed input-bundle contract](INPUT_BUNDLE.md);
- one causal, fail-closed execution path for all six observers;
- an executable `Phi` custody contract with row validity, issue time, closed
  lineage, construction hash, prefix-causality certification, and a separate
  optional representation-theorem claim;
...
- a hash-verified binding to PRAMA Protokol `0.3.0` and its numeric recertification artifact;
- executable parity, ledger, gating, and attribution-invariant tests;
- an analytic audit of whether the regulatory branch is observed, reachable
  under a declared empirical bound, or unreachable under that bound;
- an optional exhaustive circular-shift reference for Condition Report; and
- adversarial byte-exact artifact verification by deterministic custody replay.
...
The command writes one deterministic JSON artifact containing input custody, the state trajectory, all six observer outputs, localized presentations, source contracts, invariants, and the complete kernel-certification binding.

Verify a received artifact by recomputing the complete assessment:
...
This rejects noncanonical JSON, altered hashes, changed inputs, and any
byte-level difference from the deterministic replay.
