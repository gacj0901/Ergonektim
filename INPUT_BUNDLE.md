...
The manifest uses schema `ergonektim.input-bundle.v1.1`. Its packaged JSON Schema is `resources/schemas/input-bundle-manifest.schema.json`; the executable loader remains authoritative and applies additional cross-field and causal checks.
...
`causal_register.contract` declares the source, owner, role, construction, causal availability, source-validity gate, outcome independence, and independence from \(w\). Causal Link emits only when `a0_to_e1_e5_validated` is true and `experimental_only` is false. Otherwise the observer remains `instrument_indeterminate`; an experimental component can be studied outside the operational observer without being promoted into attribution.
